	...
	struct listener {
		...
		void tick_svo(const frame::tick_event &event)
		{
			...
			for (auto [entity, movement] : view.each())
			{
				...
				const int min_yaw = -45;
				const int max_yaw = 45;
				const int min_pitch = -45;
				const int max_pitch = 45;

				const int pitch_step = 1;
				const int yaw_step = 1;

				for (int yaw = min_yaw; yaw <= max_yaw; yaw += yaw_step)
				{
					float horizontalAngle = glm::radians(static_cast<float>(yaw));

					for (int pitch = min_pitch; pitch <= max_pitch; pitch += pitch_step)
					{
						float verticalAngle = glm::radians(static_cast<float>(pitch));
